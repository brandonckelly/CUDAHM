...
GibbsSampler<mfeat, pchi, dtheta>::GibbsSampler(double** meas, double** meas_unc, int ndata, dim3& nB, dim3& nT, int niter,
		int nburnin, int nthin_chi, int nthin_theta) :
		niter_(niter), nburnin_(nburnin), nthin_theta_(nthin_theta), nthin_chi_(nthin_chi)
{
	...
	Daug_ = new DataAugmentation<mfeat, pchi, dtheta>(meas, meas_unc, ndata, nB, nT);
	PopPar_ = new PopulationPar<mfeat, pchi, dtheta>(nB, nT);
...
template<int mfeat, int pchi, int dtheta>
void GibbsSampler<mfeat, pchi, dtheta>::Iterate()
{
...
template<int mfeat, int pchi, int dtheta>
void GibbsSampler<mfeat, pchi, dtheta>::Run()
{
...
template<int mfeat, int pchi, int dtheta>
void GibbsSampler<mfeat, pchi, dtheta>::Report()
{
