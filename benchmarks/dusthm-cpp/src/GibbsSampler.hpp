...
#ifndef dusthm_cpp_GibbsSampler_hpp
#define dusthm_cpp_GibbsSampler_hpp
...
public:
	...
	GibbsSampler(boost::shared_ptr<DataAugmentation<mfeat, pchi, dtheta> > Daug,
                 ...
                 int niter, int nburnin, int nthin_chi=100, int nthin_theta=1) :
    niter_(niter), nburnin_(nburnin), nthin_theta_(nthin_theta), nthin_chi_(nthin_chi)
    {
        int ndata = Daug->GetDataDim();
        _InitializeMembers(ndata);
        // construct DataAugmentation and PopulationPar objects
        Daug_ = Daug;
        PopPar_ = Theta;
        // make sure the parameter objects can talk to eachother
	...
	virtual void Report() {
        ...
        std::vector<int> naccept_chi = Daug_->GetNaccept();
...
protected:
	...
	void _InitializeMembers(int ndata) {
		// set container sizes
